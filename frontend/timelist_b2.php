...
if (isset($_POST['datepicked']))
{
    ...
    try
    {
        ...
        $db = getDB();
        $stmt = $db->prepare("SELECT sched_time FROM appointments  WHERE sched_date=:datepicked AND branch='Binakayan' AND status ='pending' OR sched_date=:datepicked AND branch='Binakayan' AND status ='confirmed' ");
