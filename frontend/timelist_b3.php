...
if (isset($_POST['datepicked']))
{
    ...
    try
    {
        ...
        $db = getDB();
        $stmt = $db->prepare("SELECT sched_time FROM appointments  WHERE sched_date=:datepicked AND branch='Langkaan' AND status ='pending' OR sched_date=:datepicked AND branch='Langkaan' AND status ='confirmed' ");
