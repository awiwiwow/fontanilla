...
$setsuccess=$setAppointment->userSetAppointment($patient_id,$branch,$sched_date,$sched_time,$reason,$status);
            ...
            if ($setsuccess )
            {
                ...
                $mail->Subject  = 'Appointment: Pending - Langkaan Branch( fontanillahalilidental.com )';
  ...
  <body>
    ...
    <div class="card">
      ...
      <h4> We have received your request. you will receive an SMS and an Email once your chosen schedule of date and time has been confirmed thank you</h4>
            ...
            $stmt = $db->prepare("SELECT sched_date FROM appointments WHERE branch = 'Langkaan' AND status='pending' OR branch = 'Langkaan' AND status = 'confirmed' OR branch = 'Langkaan' AND status = 'holiday' GROUP BY sched_date
...
<head>
  ...
  <title >Set Appointment - Langkaan Branch</title>
  <style>
  .datez{
    width: 50%;
    height: 200px;
...
//to load the j query when the whole page is already done loading.
            ...
            // $('.timecontainer').html('');$('#timelist').html(date); //call back to display in a div with an id timelist
             $('#timelist3').html(data); //call back to display in a div with an id timelist
            });
